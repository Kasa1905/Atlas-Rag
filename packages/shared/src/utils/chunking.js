...
export function chunkText(text, options) {
    ...
    while (startChar < text.length) {
        ...
        if (preserveWords && endChar < text.length) {
            ...
            const lastBoundary = Math.max(substring.lastIndexOf(' '), substring.lastIndexOf('\n'), substring.lastIndexOf('.'), substring.lastIndexOf(','), substring.lastIndexOf(';'), substring.lastIndexOf(')'));
            if (lastBoundary > 0) {
 ...
 */
export function chunkCode(code, options) {
